...
$cita = $controller->obtenerPorId($cita_id);
if (!$cita) {
...
<head>
    ...
    <title>Editar Cita - Clinica</title>
</head>
<body>
    <div>
    ...
    <?php if ($error): ?>
        <div>Error al actualizar la cita. Verifique los datos e intente nuevamente.</div>
    <?php endif; ?>
    ...
    <form method="POST" action="../../controllers/CitaController.php">
            ...
            <select name="id_paciente" id="paciente" required>
                ...
                <?php foreach ($pacientes as $paciente): ?>
                    <option value="<?= $paciente['tipo_documento'] ?>"
                            ...
                            <?= ($cita['id_paciente'] === $paciente['tipo_documento'] && $cita['documento_paciente'] === $paciente['numero_documento']) ? 'selected' : '' ?>>
                        <?= $paciente['primer_nombre'] . ' ' . $paciente['primer_apellido'] ?>
                        (<?= $paciente['tipo_documento'] . ' - ' . $paciente['numero_documento'] ?>)
                    </option>
                <?php endforeach; ?>
            </select>
            <input type="hidden" name="documento_paciente" id="documento_paciente" value="<?= $cita['documento_paciente'] ?>" required>
        </div>
        ...
        <div>
            ...
            <select name="id_profesional" id="id_profesional" required>
                ...
                <?php foreach ($medicos as $medico): ?>
                    <option value="<?= $medico['usuario_id'] ?>" <?= ((string)$cita['id_profesional'] === (string)$medico['usuario_id']) ? 'selected' : '' ?>>
        ...
        <div>
            ...
            <select name="especialidad_id" id="especialidad_id" required>
                ...
                <?php foreach ($especialidades as $especialidad): ?>
                    <option value="<?= $especialidad['especialidad_id'] ?>" <?= ((string)$cita['especialidad_id'] === (string)$especialidad['especialidad_id']) ? 'selected' : '' ?>>
        ...
        <div>
            ...
            <input type="date" name="fecha_cita" id="fecha_cita" required value="<?= $cita['fecha_cita'] ?>">
        </div>

        <div>
            <label for="hora_cita">Hora de la Cita:</label>
            <input type="time" name="hora_cita" id="hora_cita" required value="<?= substr($cita['hora_cita'], 0, 5) ?>">
        </div>

        <div>
            <label for="motivo">Motivo:</label>
            <textarea name="motivo" id="motivo" rows="4" required><?= $cita['motivo'] ?></textarea>
        </div>
